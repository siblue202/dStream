  ...
  */
  ...
  private def withAction[U](name: String, qe: QueryExecution)(action: SparkPlan => U) = {
    SQLExecution.withNewExecutionId(qe, Some(name)) {
      ...
      // jgh
      // scalastyle:off println
//      println(s"DataSet withAction qe.executedPlan: ${qe.executedPlan}")
//      println(s"DataSet withAction qe.executedPlan's class type: " +
//        s"${qe.executedPlan.getClass.getSimpleName}")
      // scalastyle:on println
  ...
  private def collectFromPlan(plan: SparkPlan): Array[T] = {
    ...
    // jgh
    // scalastyle:off println
    plan.executeCollect().map(fromRow).foreach(println)
//    println(s"plan.executeCollect().map(fromRow)'s length: " +
//      s"${plan.executeCollect().map(fromRow).length}")
//    println(s"DataSet.collectFromPlan : ${plan.executeCollect().map(fromRow)}")
    // scalastyle:on println
