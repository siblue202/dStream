...
object SparkPlan {
  ...
  // jgh start
  // <------------------ Variables of Dstream-Extension START--------------------------->
  // These variables are used by rapids to calculate the Cost value.
  // They are set in MicroBatchExecution when the MB is constructed.
  ...
  // Time of current micro-batch time
  var currentMBTime: Long = 0
  // inflection point (lmStream)
  var inflectionPoint: Double = 0.0

  def getCurrentMBSize: Double = {
    currentMBSize
  }
  def getTotalMBSize: Double = {
    totalMBSize
  }
  ...
  def setTotalMBSize(curSize: Double): Unit = {
    totalMBSize = curSize
  }
  def setNumDSOfMB(curSize: Int): Unit = {
    numDSOfMB = curSize
  }
  def setNumPartitions(curSize: Int): Unit = {
    numPartitions = curSize
  }
  def setMBTime(curTime: Long): Unit = {
    inflectionPoint = (currentMBSize / curTime) + curTime
    currentMBTime = curTime
  }
  // jgh end
}
...
abstract class SparkPlan extends QueryPlan[SparkPlan] with Logging with Serializable {
   ...
   */
    // jgh: we need to replace this to backup!!!!!!!!
...
//    println(s"call ${this.getClass.getSimpleName}'s doExecute()! on execute() in SparkPlan class")

    val result = doExecute()
//    println(s"done ${this.getClass.getSimpleName}'s doExecute()! on execute() in SparkPlan class")
...
//  }
  ...
  final def prepare(): Unit = {
    ...
    // jgh
    // scalastyle:off println
//    println(s"what is children?: ${children} on prepare() in SparkPlan")
    // scalastyle:on println
  ...
  def executeCollect(): Array[InternalRow] = {
    ...
    // jgh
    // scalastyle:off println
//    println(s"SparkPlan executeCollect() byteArrayRdd's class: " +
//      s"${byteArrayRdd.getClass.getSimpleName}")
    // scalastyle:on println
