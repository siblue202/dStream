...
class MicroBatchExecution(
  ...
  // lsy start
  // <------------------ Variables of Q-Spark-Extension START--------------------------->
  // * MB = Micro-Batch
  // * DS = Dataset; minimum unit of data which is a collection of raw records. (e.g., single file)
  // * One single MB can contain one or more DSs.
  // Thanks to LSY

  // Size of the single current MB.
  var currentMBSize: Double = 0.0
  // Size of total MB during entire query execution.
  var totalMBSize: Double = 0.0
  // Number of datasets (files) within current MB.
  var numDSOfMB: Int = 0
  // <------------------ Variables of Q-Spark-Extension END----------------------------->
  // lsy end
  ...
  protected def runActivatedStream(sparkSessionForStream: SparkSession): Unit = {
    ...
    triggerExecutor.execute(() => {
      if (isActive) {
        // jgh start
        // for lmStream test
        var mbStartTime: Long = System.currentTimeMillis()
        // jgh end
        ...
        reportTimeTaken(":q" +
          ...
          // jgh start
          // update micro-batch metrics
          SparkPlan.setCurrentMBSize(currentMBSize)
          SparkPlan.setTotalMBSize(totalMBSize)
          SparkPlan.setNumDSOfMB(numDSOfMB)
          SparkPlan.setNumPartitions(SQLConf.get.defaultNumShufflePartitions)
          // jgh end
        ...
        val mbEndTime: Long = System.currentTimeMillis()
        val currentMBRunTime: Long = mbEndTime - mbStartTime

        SparkPlan.setMBTime(currentMBRunTime)
        ...
        if (isCurrentBatchConstructed) {
//          logInfo(s"currentBatchID : ${currentBatchId}, MBSize : ${currentMBSize}") // jgh
          currentBatchId += 1
          // jgh start
          // reset value
          uniqueSources.map {
            case (s: SupportsAdmissionControl, limit) =>
              s match {
                case f: FileStreamSource =>
//                  logInfo(s"JGH: reset!!!!!")
                  f.setCurrentMBSize(0.0)
                  f.setNumDSOfMB(0)
              }
          }
//          logInfo(s"reset variables that used to calculate Cost of Operater")
          // jgh end
  ...
  private def constructNextBatch(noDataBatchesEnabled: Boolean): Boolean = withProgressLocked {
    ...
    // lsy start
    // Initialize values that are maintained per MB considering numerous sources per query.
//    currentMBSize = 0.0
//    numDSOfMB = 0
    // lsy end
    ...
    // jgh: val to var
    var (nextOffsets, recentOffsets) = uniqueSources.toSeq.map {
      case (s: SupportsAdmissionControl, limit) =>
//        logInfo(s"JGH: case SupportsAdmissionControl") // jgh
        ...
        reportTimeTaken("latestOffset") {
          ...
          s match {
            case f: FileStreamSource =>
//              logInfo(s"JGH: FileStreamSource") // jgh
              // Since there might be numerous sources per query;
              // we need to maintain and keep add to some variables.
              // Pass essential infos before constructing MB.
//              f.currentMBSize = currentMBSize
//              f.numDSOfMB = numDSOfMB
          }
          // lsy end
          ((s, Option(next)), (s, Option(latest)))
        }
      case (s: Source, _) =>
//        logInfo(s"JGH: case Source") // jgh
        updateStatusMessage(s"Getting offsets from $s")
...
//        logInfo(s"JGH: case MicroBatchStream") // jgh
        updateStatusMessage(s"Getting offsets from $s")
        reportTimeTaken("latestOffset") {
    ...
    // lsy start
    // Get info after constructing MB.
    uniqueSources.map {
      case (s: SupportsAdmissionControl, limit) =>
        s match {
          case f: FileStreamSource =>
//            logInfo(s"JGH: jebal jebal")
            // Since there might be numerous sources per query, we need to keep add to metrics;
            // instead of assigning new value to them.
//            currentMBSize += f.getCurrentMBSize()
//            numDSOfMB += f.getNumDSOfMB()
            currentMBSize = f.getCurrentMBSize()
            numDSOfMB = f.getNumDSOfMB()
//            logInfo(s"JGH: currentMBSize: ${currentMBSize}")
//            logInfo(s"JGH: numDSOfMB: ${numDSOfMB}")
        }
//        currentMBSize += s.getCurrentMBSize() // jgh
//        numDSOfMB += s.getNumDSOfMB() // jgh
    }
    // lsy end
...
//        println(s"sink type: ${sink.getClass.getSimpleName}")
        ...
        // jgh
        // scalastyle:off println
//        println(s"lastExecution.executedPlan type: ${lastExecution.executedPlan
