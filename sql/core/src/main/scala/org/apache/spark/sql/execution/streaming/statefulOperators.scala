...
trait StateStoreReader extends StatefulOperator {
  override lazy val metrics = Map(
    ...
    "duration" -> SQLMetrics.createNanoTimingMetric(sparkContext,
      "time duration of doExecute()") // jgh
  )
}
...
//        val startTimeMs = System.currentTimeMillis() // jgh
        // jgh
        // scalastyle:off println
        println(s"before hasNext is called! on StateStoreRestoreFunction " +
          s"in statefulOperators.scala")
        ...
        if (!hasInput && keyExpressions.isEmpty) {
          ...
          // jgh: insert val to match return type
          val result = store.iterator().map(_.value)
          duration += System.nanoTime - startTimeNs // jgh
//          duration += (System.currentTimeMillis() - startTimeMs) // jgh
          // scalastyle:off println
          println(s"stateStoreRestore's duration: ${duration}")
//          println(s"stateStoreRestore's NanoDuration: ${System.nanoTime - startTimeNs}")
          // scalastyle:on println
          result // jgh
        } else {
          // jgh: insert val to match return type
          val result = iter.flatMap { row =>
      ...
      Some(session.streams.stateStoreCoordinator)) { (store, iter) =>
        ...
        outputMode match {
          ...
          case Some(Complete) =>
            // jgh start
            // scalastyle:off println
//            println(s"StateStoreSaveExec: Complete")
            // scalastyle:on println
            // jgh end
          ...
          case Some(Append) =>
            // jgh start
            // scalastyle:off println
//            println(s"StateStoreSaveExec: Append")
            // scalastyle:on println
            // jgh end
            allUpdatesTimeMs += timeTakenMs {
            ...
            // scalastyle:off println
            println(s"Append UpdateTime: ${allUpdatesTimeMs.value}@${numUpdatedStateRows.value}")
            // scalastyle:on println
...
//                allRemovalsTimeMs += NANOSECONDS.toMillis(System.nanoTime - removalStartTimeNs)
                allRemovalsTimeMs += System.nanoTime - removalStartTimeNs // jgh
                commitTimeMs += timeTakenMs { stateManager.commit(store) }
                // jgh start
                // scalastyle:off println
//                println(s"Append remove&commitTime: ${allRemovalsTimeMs.value +
//                  commitTimeMs.value}")
                // scalastyle:on println
                // jgh end
                setStoreMetrics(store)
          ...
          case Some(Update) =>
            // jgh start
            // scalastyle:off println
//            println(s"StateStoreSaveExec: Update")
            // scalastyle:on println
            // jgh end
